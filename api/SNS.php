...
class SNS extends Base {
    ...
    public static function getToken($appid, $appsecret) {
        $data = self::doGet(self::API_GET_TOKEN, array('appid' => $appid, 'appsecret' => $appsecret));
     ...
     */
    public static function getPersistentCode($tmpAuthCode) {
        $data = self::doPost(self::API_GET_PERSISTENT_CODE, array('tmp_auth_code' => $tmpAuthCode));
     ...
     */
    public static function getSNSToken($openid, $persistentCode) {
        $data = self::doPost(self::API_GET_SNS_TOKEN, array('openid' => $openid, 'persistent_code' => $persistentCode));
    ...
    public static function getUserInfo($snsToken) {
        $data = self::doGet(self::API_GET_USERINFO, array('sns_token' => $snsToken));
